...
import { ReqPostMarca, ReqPutMarca, RespPostMarca } from "@api/interface/api-marca.interface";
...
export class ApiProducto extends HttpBase {
    getProducto(page: number): Observable<Inbox<{ id: number; detalle: string}> >{
        const endpoint = `${API.apiProducto}/pagina/${page}`;
        return this.http.get<ResponseInbox<{id: number; nombre: string; detalle: string;}>>(endpoint).pipe(map(resp => ({
